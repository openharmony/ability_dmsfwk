...
void FuzzDistributedOperation(const uint8_t* data, size_t size)
{
    ...
    Parcel parcelTemp;
    parcelTemp.WriteBuffer(data, size);
    parcelTemp.RewindRead(0);
    operation.ReadFromParcel(parcelTemp);

    DistributedSchedule::DistributedOperation operationTemp(operation);

    if (operationTemp == operation) {
        return;
    }
}
}
